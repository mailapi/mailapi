...
- Mail API `200` and `202` both map to a `true` return from `wp_mail()`. They
  differ in whether submission execution completed before the response, but
  neither means the mail reached a recipient.
